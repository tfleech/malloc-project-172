...
static int add_range(const malloc_impl_t* impl, range_t** ranges, char* lo,
  ...
  if ((char*)mem_heap_lo() > lo) {
    malloc_error(tracenum, opnum, "Below Heap Low");
    return 0;
  }

  if (hi > (char*)mem_heap_hi()) {
    malloc_error(tracenum, opnum, "Above Heap High");
    return 0;
  }
  ...
  for (; p != NULL; p = p->next) {
    ...
    if ((lo <= hi_b && lo >= lo_b) | (hi <= hi_b && hi >= lo_b)) {
      malloc_error(tracenum, opnum, "Overlapping Blocks");
      return 0;
    }
  }
...
static void remove_range(range_t** ranges, char* lo) {
  ...
  // Check if payload is at the head
  if (prevpp->lo == lo) {
    *ranges = prevpp->next;
    free(prevpp);
    return;
...
int eval_mm_valid(const malloc_impl_t* impl, trace_t* trace, int tracenum) {
  ...
  for (i = 0; i < trace->num_ops; i++) {
    ...
    case ALLOC:  // malloc
      ...
      // Set first and last byte to index
      char* end = (p + size - 1);
      *end = index;
      *p = index;
    ...
    case REALLOC:  // realloc
      ...
      //verify data was correctly copied and then adjust markers to new size
      if ((*newp == index) && (*(newp + oldsize - 1) == index)) {
        *(newp + oldsize - 1) = 0x00000000;
        *(newp + size - 1) = index;
      } else {
        malloc_error(tracenum, i, "Data corrupted during realloc");
        return 0;
      }
